...
/**
 * Bootstraps a new game round and starts background music.
 * @fires HTMLMediaElement#play
 * @returns {void}
 * @example
 * // Start a new round from a "Play" button:
 ...
 * @example
 * // On "Replay" button
 * document.getElementById('replay').addEventListener('click', reloadGame);
 */
function reloadGame() {
  document.getElementById('endscreen').classList.add('d-none')
  startGame()
...
/**
 * Stops the current game session and shows end screen after a short delay.
 * @returns {void}
 */
function stopGame() {
  clearAllIntervals();
 ...
 * Renders either the "game won" or a random "game over" screen.
 * @returns {void}
 * @see renderGameWonScreen
 * @see renderRandomGameOverScreen
 */
function showEndScreen() {
  let endscreen = document.getElementById("endscreen");
  if (world.gameWon) {
